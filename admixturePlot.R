...
admixturePlot <- function(infile, outfile, k, ...){
  ...
  tAdmixIn <- t(as.matrix(admixIn[2:nrow(admixIn), 6:ncol(admixIn)-1]))
  mycol <- brewer.pal(k, "Paired")
  
  pdf(outfile, 11, 8.5)
  par(mar=c(12,5,2,2))
  barplot(tAdmixIn, col=mycol, space=0.2, names.arg=paste(admixIn[2:nrow(admixIn), 2], admixIn[2:nrow(admixIn), 4], sep="_"), las=2, ylab="Admixture")
  dev.off()
}
